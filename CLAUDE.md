...
- THE GUN CABINET — an unannounced quest (owner round 20, `WorldState` "GUN CABINETS" block,
  ...
  first (`CABINET_LOOTED_CHANCE` 30% / 35% per run, sticky; a charred flat's is gone) — the LOOTED
  look, 40% a few dropped rounds. State is cross-run + saved (`gun_cabinets`, cleared by new_game):
  locked / open / smashed / open_empty / smashed_empty / looted; the non-locked looks are OVERLAYS
  generated by `tools/art/living_room_variants.py` (`_cabinet_overlays`, every run look) and laid
  over the module art (live + balcony backdrop). The art (owner round 20b — the first redraw read as
  "a trophy case for billiards sticks"): a walnut cabinet, a glazed door with two slanting
  reflections, THREE LONG GUNS in the rack (a scoped rifle, a double-barrel shotgun, a lever-action)
  held by a padlocked cable through their trigger guards (why you can't take them), the HANDGUN on a
  felt shelf below, a brass keyhole. Smashed = jagged pieces of every size left in the frame, the
