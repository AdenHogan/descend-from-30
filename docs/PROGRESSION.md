...
- **The offer** (`WorldState.finish_session`, called at the arc end in `game.gd` / `lobby_exit.gd`):
  ...
  **acquired this session** (`WorldState.session_perks`: every merchant upgrade taken + every run
  ...
  New Game) — uniformly unless you hold perk luck (below). A perk already kept is never offered. So to keep a perk you must farm it that session.
...
- **Targeting = farming (owner, round 6 — confirmed):** the offer is ONLY ever perks you selected
  across that game's three runs — never padded, never guaranteed. Wanting a particular perk means
  replaying and picking it up again, which is the point ("incentivises replaying to get as many
  options to target-farm desired permanent perks").
- **DESIRABILITY + perk luck (owner, round 6 — "a little meta"):** every perk carries a
  **desirability `d` 1-5** beside its rarity `w` (the column lives on the pools: `UPGRADE_POOL` in
  world_state.gd, `RUN_BOONS` in progression.gd; the readable list is **docs/PERKS.md**, generated by
  `tools/perk_table.tscn` and checked current by `progression_test`). The new merchant upgrade
  **Fortune's Favour** (`U_fortune`, `perk_luck` +1, **w1 — the rarest tier** (owner, round 7: ~2% of shop visits, ~1 game in 4; was w3: 6%, 3 in 5), keep forever 400) tilts EVERY offer toward
  high-`d` perks — the merchant's pair, milestone boons and the end-of-game Valour offer — by
  `DESIRE_TILT ^ (luck × (d − 3))` (1.6: a d5 perk ~2.6× likelier, a d1 ~0.4×;
  `Progression.desire_weight` / `weighted_draw`). With no luck every draw runs its original code path
  unchanged (the end offer stays uniformly random). Taken early it shapes the rest of that game's
  offers; kept forever, every game's. Measured: merchant pairs average d 3.33 vs 2.98, the top-rated
  perk reaches the end offer ~180/200 vs ~100/200 (from a 6-perk pool).
