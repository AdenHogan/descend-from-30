...
const BAND_TOP := 207.0      # room.ROOM_BAND_TOP — the camera's top edge
const BAND_BOTTOM := 367.0   # BAND_TOP + ROOM_BAND_H — never draw into the flat below
...
func _draw() -> void:
	...
	# --- the ceiling slab: floorboards of the flat above, the mass, the plaster ceiling edge ---
	draw_rect(Rect2(x0, BAND_TOP, x1 - x0, yc - BAND_TOP), SECTION)
	draw_rect(Rect2(x0, BAND_TOP, x1 - x0, 3.0), BOARD)
	draw_rect(Rect2(x0, BAND_TOP + 3.0, x1 - x0, 1.0), BOARD_LO)
	var bx := x0 + 7.0
	while bx < x1:                                  # board ends, staggered — reads as a floor
		draw_rect(Rect2(bx, BAND_TOP, 1.0, 3.0), BOARD_LO)
		bx += 23.0 if int(bx) % 2 == 0 else 31.0
	draw_rect(Rect2(x0, BAND_TOP + 5.0, x1 - x0, 1.0), SECTION_LT)
